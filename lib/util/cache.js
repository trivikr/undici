...
/**
 * @param {import('../../types/dispatcher.d.ts').default.DispatchOptions} opts
 * @param {Record<string, string[] | string> | undefined} [headers=opts.headers]
 */
function makeCacheKey (opts, headers = opts.headers) {
  return {
    origin: opts.origin.toString(),
    method: opts.method,
    path: getCachePath(opts),
    headers
  }
}

/**
 * Creates a cheap, collision-resistant key for request deduplication lookup.
 * This only includes origin, method, and path so header processing can stay lazy.
 * @param {import('../../types/dispatcher.d.ts').default.DispatchOptions} opts
 * @returns {string}
 */
function makeDeduplicationLookupKey (opts) {
  const origin = opts.origin.toString()
  const method = opts.method
  const path = getCachePath(opts)

  return `${origin.length}:${origin}${method.length}:${method}${path.length}:${path}`
}
