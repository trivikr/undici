...
function writeH2 (client, request) {
  ...
  let completeWithoutTrailersImmediate = null

  const clearCompleteWithoutTrailersImmediate = () => {
    if (completeWithoutTrailersImmediate !== null) {
      clearImmediate(completeWithoutTrailersImmediate)
      completeWithoutTrailersImmediate = null
    }
  }

  const removeRequestStreamListeners = () => {
    clearCompleteWithoutTrailersImmediate()
    stream.off('error', noop)
  ...
  const onEnd = () => {
    ...
    if (responseReceived) {
      completeWithoutTrailersImmediate = setImmediate(() => {
        completeWithoutTrailersImmediate = null
        releaseRequestStream()
