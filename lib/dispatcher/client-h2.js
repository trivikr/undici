...
function failUpgradeStream (state, err) {
  ...
  abortRequestH2.call(state, err, true)
}
...
function finalizeRequestH2 (resetPendingIdx = false) {
  const state = this

  const { client, request } = state

  if (request[kRequestStreamState] !== state) {
    return
  }

  request[kRequestStreamState] = null
...
function abortRequestH2 (err, resetPendingIdx = false) {
  const state = this?.request ? this : this[kRequestStreamState]

  if (state == null) {
    return
  }

  const { body, client, request } = state
  ...
  if (state.stream != null) {
    ...
    client[kOnError](err)
    finalizeRequestH2.call(state, resetPendingIdx)
  }
...
function requestStreamH2 (state, headers, options) {
  const { session } = state
  ...
  try {
    ...
    abortRequestH2.call(state, wrappedErr)
...
function writeH2 (client, request) {
  ...
  request[kRequestStreamState] = state

  try {
    // We are already connected, streams are pending.
    // We can call on connect, and wait for abort
    request.onRequestStart(abortRequestH2, null)
  } catch (err) {
    util.errorRequest(client, request, err)
  }
  ...
  if (upgrade || method === 'CONNECT') {
    ...
    if (upgrade === 'websocket') {
      ...
      stream = requestStreamH2(state, headers, { endStream: false, signal })
      if (stream == null) {
        session.unref()
        return false
      }
  ...
  stream = requestStreamH2(state, headers, { endStream: shouldEndStream, signal })
  if (stream == null) {
    return false
  }
  stream[kHTTP2Stream] = true
...
function onEnd () {
  ...
  if (state.responseReceived) {
    ...
    abortRequestH2.call(state, new InformationalError('HTTP/2: stream half-closed (remote)'), true)
  }
}
...
function onFrameError (type, code) {
  ...
  releaseRequestStream(stream)
  abortRequestH2.call(state, new InformationalError(`HTTP/2: "frameError" received - type ${type}, code ${code}`))
}
...
function writeBodyH2 () {
  const stream = this
  const state = stream[kRequestStreamState]
  const { body, client, contentLength, expectsPayload, request } = state
...
function writeBuffer (state, h2stream, body, client, request, socket, contentLength, expectsPayload) {
  try {
    ...
    abortRequestH2.call(state, error)
  }
}

function writeStream (state, socket, expectsPayload, h2stream, body, client, request, contentLength) {
...
async function writeBlob (state, h2stream, body, client, request, socket, contentLength, expectsPayload) {
  try {
...
async function writeIterable (state, h2stream, body, client, request, socket, contentLength, expectsPayload) {
