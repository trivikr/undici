...
class EnvHttpProxyAgent extends DispatcherBase {
  ...
  [kDispatch] (opts, handler) {
    const agent = this.#getProxyAgentForOrigin(opts.origin)
  ...
  #getProxyAgentForOrigin (origin) {
    if (this.#noProxyChanged) {
      this.#parseNoProxy()
    }

    const originKey = String(origin)
    let agent = this.#proxyAgentCache.get(originKey)
    if (agent) {
      return agent
    }

    const url = new URL(origin)
    agent = this.#getProxyAgentForUrl(url)
    this.#proxyAgentCache.set(originKey, agent)
    return agent
  }
